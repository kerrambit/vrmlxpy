...
TEST_CASE("Paretgregrehg", "[parsing][valid]") {
    ...
    if (it2 != descriptorMap.end()) {
        ...
        if (validationResult.has_value()) {
            ...
            auto result = vrml_proc::traversor::handler::WorldInfoHandler::Handle<to_stl::conversion_context::MeshTaskConversionContext>({ parseResult.value().at(0), manager, false, vrml_proc::math::TransformationMatrix(), config }, to_stl::conversion_context::CreateActionMap(), nd);
            if (result.has_error()) {
...
TEST_CASE("Parse VRMLFile From File - Valid Input - Actin", "[parsing][valid][fromfile]") {
    ...
    CHECK(TraverseVrmlFileToMeshTask(parseResult, manager, 5, std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, 5, config));
...
TEST_CASE("Parse VRMLFile From File - Valid Input - Nucleus", "[parsing][valid][fromfile]") {
    ...
    GENERATE_TEST_OUTPUT_FILENAME(filepath);
    CHECK(TraverseVrmlFileToMeshTask(parseResult, manager, 2, std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, 2, config));
    CHECK(HaveSimiliarSizes(std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, std::filesystem::path(ReadTestInfo().baseExpectedOutputPath) / filepath, 0));
}
...
TEST_CASE("Parse VRMLFile From File - Valid Input - Tubulin", "[parsing][valid][fromfile]") {
    ...
    GENERATE_TEST_OUTPUT_FILENAME(filepath);
    CHECK(TraverseVrmlFileToMeshTask(parseResult, manager, 399, std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, 399, config));
    CHECK(HaveSimiliarSizes(std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, std::filesystem::path(ReadTestInfo().baseExpectedOutputPath) / filepath, 0));
}
...
//
//    vrml_proc::parser::VrmlNodeManager manager;
//    auto parseResult = ParseVrmlFile(std::filesystem::path(ReadTestInfo().baseInputPath) / std::filesystem::path(ReadTestInfo().testFiles.at("TILES")), manager);
//    REQUIRE(parseResult);
//
//    vrml_proc::core::config::VrmlProcConfig config;
//    config.ignoreUnknownNode = true;
//
//    GENERATE_TEST_OUTPUT_FILENAME(filepath);
//    CHECK(TraverseVrmlFileToMeshTask(parseResult, manager, 64611, std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, 64611, config));
//    CHECK(HaveSimiliarSizes(std::filesystem::path(ReadTestInfo().baseOutputPath) / filepath, std::filesystem::path(ReadTestInfo().baseExpectedOutputPath) / filepath, 0));
//}