    ...
    for (size_t i = 0; i < indicides.size(); ++i) {
      if (indicides[i] == -1) {
        ...
        if (end - start == 3) {
          ...
          auto vertex1 = vrml_proc::math::cgal::CGALPoint(points[indicides[start]].x, points[indicides[start]].y,
                                                          points[indicides[start]].z);

          auto vertex2 = vrml_proc::math::cgal::CGALPoint(
              points[indicides[start + 1]].x, points[indicides[start + 1]].y, points[indicides[start + 1]].z);

          auto vertex3 = vrml_proc::math::cgal::CGALPoint(
              points[indicides[start + 2]].x, points[indicides[start + 2]].y, points[indicides[start + 2]].z);

          mesh->add_face(mesh->add_vertex(matrix.transform(vertex1)), mesh->add_vertex(matrix.transform(vertex2)),
                         mesh->add_vertex(matrix.transform(vertex3)));
        } else {
          return cpp::fail(
              std::make_shared<error::IndexedFaceSetCalculatorError>() << std::make_shared<
                  vrml_proc::core::error::UnsupportedOperationError>(
