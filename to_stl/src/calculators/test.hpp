...
CalculatorResult
createMeshFromData(std::reference_wrapper<const std::vector<float>> data, int id) {
...
CalculatorResult
createMeshFromParams(int id, const std::vector<int>& params) {
...
CalculatorResult
createMeshFromFloat(int id, float value) {
...
int taskflow_test() {
...

    {
        ...
        taskVector->Add([=]() {
            return createMeshFromData(std::cref(meshInput1), 1);
            });
    }

    {
        ...
        taskVector->Add([=]() {
            return createMeshFromParams(2, params);
            });
    }

    {
        ...
        taskVector->Add([=]() {
            return createMeshFromFloat(3, singleValue);
            });
    }
