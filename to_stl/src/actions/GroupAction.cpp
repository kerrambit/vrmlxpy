  ...
  namespace action {
    ...
    std::shared_ptr<to_stl::conversion_context::MeshTaskConversionContext> GroupAction::Execute() {
      ...
      for (const auto& child : m_children) {
        if (child != nullptr) {
          std::shared_ptr<to_stl::conversion_context::MeshTaskConversionContext> meshContextPtr =
              std::dynamic_pointer_cast<to_stl::conversion_context::MeshTaskConversionContext>(child);
