...
namespace vrmlxpy {
    ...
    void PrintVersion() {
        std::cout << "vrmlxpy 0.1 (tostl 0.1; vrmlproc 1.0)" << std::endl; // TODO: this will be changed.
    }
    ...
    bool ConvertVrmlToStl(const std::string& inputFilename, const std::string& outputFilename, const std::string& configFilename) {
        ...
        std::cout << "Configuration file " << std::filesystem::path(configFilename).string() << " was succesfully parsed." << std::endl;
        ...
        auto convertResult = vrml_proc::traversor::VrmlFileTraversor::Traverse<to_stl::conversion_context::MeshTaskConversionContext>({ parseResult.value(), manager, config }, to_stl::conversion_context::CreateActionMap());
    ...
    bool ConvertVrmlToStl(const std::string& inputFilename, const std::string& outputFilename) {
        ...
        if (convertResult.has_error()) {
            std::cout << "Caught an application error:\n" << convertResult.error()->GetMessage() << std::endl;
            std::shared_ptr<vrml_proc::traversor::error::FileTraversorError> error = std::dynamic_pointer_cast<vrml_proc::traversor::error::FileTraversorError>(convertResult.error());
            if (error != nullptr) {
...
PYBIND11_MODULE(vrmlxpy, m) {
    ...
    m.def("convert_vrml_to_stl",
        py::overload_cast<const std::string&, const std::string&>(&vrmlxpy::ConvertVrmlToStl),
        "Converts a VRML file to STL",
    ...
    m.def("convert_vrml_to_stl",
        ...
        "Converts a VRML file to STL with a config file",
        py::arg("input_filename"), py::arg("output_filename"), py::arg("config_filename"));
