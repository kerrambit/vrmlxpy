...
void vrml_proc::parser::VrmlNodeManager::AddDefinitionNode(const std::string& id,
                                                           const vrml_proc::parser::VrmlNode& node) {
  vrml_proc::core::logger::LogDebug(vrml_proc::core::utils::FormatString("Add DEF node with id <", id, "> into map."),
                                    LOGGING_INFO);
  vrml_proc::core::logger::LogDebug(vrml_proc::core::utils::FormatString("Address of VRML node: <", &node, ">."),
                                    LOGGING_INFO);
  ...
  if (!result.second) {
    vrml_proc::core::logger::LogDebug(
        vrml_proc::core::utils::FormatString(
            "Id <", id, "> was already present in the map. The value was replaced with the new entity."),
        LOGGING_INFO);
  }
}
...
vrml_proc::parser::VrmlNodeManager::GetDefinitionNode(const std::string& id) const {
  ...
  vrml_proc::core::logger::LogDebug(
      vrml_proc::core::utils::FormatString("DEF node with id <", id, "> was not found in the map!"), LOGGING_INFO);
  return {};
}
