...
namespace vrml_proc::traversor::handler::ColorHandler {
  ...
  Handle(vrml_proc::traversor::FullParsedVrmlNodeContext context,
    ...
    if (validationResult.has_error()) {
      std::shared_ptr<vrml_proc::core::error::Error> error =
          std::make_shared<vrml_proc::traversor::error::NodeTraversorError>(validationResult.error(), context.node);
    ...
    return vrml_proc::traversor::utils::ConversionContextActionExecutor::TryToExecute<ConversionContext>(
        actionMap, context.node.header, {std::cref(cachedColor)}, {});
  }
  ...
  Handle(vrml_proc::traversor::FullParsedVrmlNodeContext context,
    ...
    std::any cachedColor = nd.GetField<std::reference_wrapper<vrml_proc::parser::Vec3fArray>>("color");
    return vrml_proc::traversor::utils::ConversionContextActionExecutor::TryToExecute<ConversionContext>(
        actionMap, nd.GetId(), {std::cref(cachedColor)}, {});
  }
}  // namespace vrml_proc::traversor::handler::ColorHandler