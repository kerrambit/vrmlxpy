...
namespace vrml_proc {
	namespace traversor {
		namespace validator {
			...
			class TextureCoordinateNodeValidator : public NodeValidator {
			public:

				TextureCoordinateNodeValidator(const vrml_proc::parser::VrmlNode& node)
					: m_node(node) {}

				cpp::result<void, std::shared_ptr<error::NodeValidationError>> Validate() override {

					LOG_INFO() << "Validate TextureCoordinate VRML node.";
					...
					auto pointResult = vrml_proc::traversor::validator::NodeValidator::ExtractFieldByNameWithValidation<vrml_proc::parser::Vec2fArray>("point", m_node.fields);
				...
				std::reference_wrapper<const vrml_proc::parser::Vec2fArray> GetCachedPoint(const vrml_proc::parser::Vec2fArray& defaultValue) {
			...
			private:
				...
				std::optional<std::reference_wrapper<const vrml_proc::parser::Vec2fArray>> m_point = std::optional<std::reference_wrapper<const vrml_proc::parser::Vec2fArray>>{};
			};
		}
	}
}
