...
vrml_proc::traversor::VrmlFileTraversor::Traverse(
    const FullParsedVrmlFileContext& context,
  ...
  for (const auto& root : context.file) {
    vrml_proc::core::logger::LogInfo(
        vrml_proc::core::utils::FormatString("Found root node of type <", root.header, ">."), LOGGING_INFO);
    ...
    cpp::result<std::shared_ptr<ConversionContext>, std::shared_ptr<vrml_proc::core::error::Error>> result =
        vrml_proc::traversor::VrmlNodeTraversor::Traverse<ConversionContext>(
            vrml_proc::traversor::FullParsedVrmlNodeContext(root, context.manager, false,
                                                            vrml_proc::math::TransformationMatrix()),
            actionMap);

    if (result.has_error()) {
      vrml_proc::core::logger::LogError(
          vrml_proc::core::utils::FormatString("While traversing root node <", root.header, "> error occured!"),
          LOGGING_INFO);
    ...
    if (result.value() == nullptr) {
      vrml_proc::core::logger::LogError(
          vrml_proc::core::utils::FormatString("While traversing root node <", root.header,
                                               "> unxpected internal error occured!"),
          LOGGING_INFO);
      auto nullPointerError = std::make_shared<vrml_proc::core::error::NullPointerError>();
      auto fileError = std::make_shared<vrml_proc::traversor::error::FileTraversorError>(nullPointerError, root);
      return cpp::fail(fileError);
    }
...
namespace vrml_proc {
  namespace traversor {
    namespace VrmlFileTraversor {
      template VRMLPROCESSING_API cpp::result<std::shared_ptr<vrml_proc::conversion_context::MeshConversionContext>,
                                              std::shared_ptr<vrml_proc::core::error::Error>>
      Traverse<vrml_proc::conversion_context::MeshConversionContext>(
          const FullParsedVrmlFileContext& context,
