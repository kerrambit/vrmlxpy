...
TEST_CASE("ExtractVrmlNodeFromVariantExtended - UseNode", "[valid]") {
    ...
    auto managerFound = manager.GetDefinitionNode("id");
    REQUIRE(managerFound.has_value());
    CHECK(&wrapper.get() == &(managerFound.value().get()));
}
...
TEST_CASE("ExtractVrmlNodeExtended - USENode", "[valid]") {
    ...
    auto managerFound = manager.GetDefinitionNode("id");
    REQUIRE(managerFound.has_value());
    CHECK(&(managerFound.value().get()) == &(result.value().get()));
}
